...
void MainWindow::readyReadData()
{
    ...
    QFile imageFile;
    /* 保存到当前路径，名称为"下载的.jpg" */
    imageFile.setFileName(QCoreApplication::applicationDirPath() + "/下载的.jpg");
    ...
    if (data.isEmpty())
    {
        qDebug() << "data is null, please try it again!";
        return;
    }
    ...
    {
        qDebug() << "not JPG data, please try it again!";
        return;
    }
...
void MainWindow::replyFinished()
{
    ...
    /* 判断当前执行程序下的图像是否下载完成 */
    QFile imageFile(QCoreApplication::applicationDirPath() + "/下载的.jpg");
    if (imageFile.exists())
    {
        /* 显示下载的图像 */
